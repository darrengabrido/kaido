...
struct RouteDetailView: View {
    ...
    /// Mirrors `routeHeaderButton`'s circle-icon shape, but needs its own view rather than
    /// reusing that helper: it swaps between two different icons/actions depending on
    /// `route.sharedCommunityRouteId` and shows a spinner mid-request, none of which the plain
    /// toggle-style helper supports.
    @ViewBuilder
    private var communityShareHeaderButton: some View {
        if route.sharedCommunityRouteId != nil {
            Button {
                ...
                .background(Color.routeTeal.opacity(0.18), in: Circle())
            }
            .buttonStyle(.plain)
