...
struct BikeShowcaseCard: View {
    ...
    var body: some View {
        ...
        .background {
            RoundedRectangle(cornerRadius: 28)
                .fill(
                    LinearGradient(
                        colors: [
                            Color.vectorIndigo.opacity(0.58),
                            Color.vectorMidnight.opacity(0.98)
                        ],
        ...
        .overlay {
            RoundedRectangle(cornerRadius: 28)
                .stroke(Color.vectorViolet.opacity(0.2), lineWidth: 1)
        }
    ...
    private var profileTag: some View {
        Label(profile.bikeType.title, systemImage: profile.bikeType.systemImage)
            ...
            .padding(.vertical, 6)
            .background(Color.vectorInk.opacity(0.1), in: Capsule())
    }
...
private struct BikeSilhouette: View {
        ...
        GeometryReader { proxy in
            ...
            ZStack {
                ...
                BikeFrameShape(geometry: geometry)
                    .stroke(
                        Color.vectorInk.opacity(0.92),
                        style: StrokeStyle(lineWidth: 4, lineCap: .round, lineJoin: .round)
                    )

                BikeControlsShape(geometry: geometry)
                    .stroke(
                        Color.vectorInk.opacity(0.9),
                        style: StrokeStyle(lineWidth: 3, lineCap: .round, lineJoin: .round)
                    )

                BikeDrivetrainShape(geometry: geometry)
                    .stroke(
                        Color.vectorDim.opacity(0.9),
                        style: StrokeStyle(lineWidth: 2, lineCap: .round, lineJoin: .round)
                    )

                Capsule()
                    .fill(Color.vectorInk.opacity(0.92))
                ...
                Capsule()
                    .fill(Color.vectorInk.opacity(0.9))
                    .frame(width: 15, height: 4)
                    .rotationEffect(.degrees(42))
                    .position(geometry.handlebarGrip)

                Circle()
                    .fill(Color.vectorInk.opacity(0.82))
                    .frame(width: 7, height: 7)
                ...
                if type == .electric || type == .cargo {
                    ...
                    Circle()
                        .stroke(Color.vectorInk.opacity(0.88), lineWidth: 2)
    ...
    private func wheel(center: CGPoint, radius: CGFloat) -> some View {
        ZStack {
            Circle()
                .stroke(Color.vectorDim.opacity(0.85), lineWidth: 3)
                .frame(width: radius * 2, height: radius * 2)
                .position(center)

            Circle()
                .stroke(Color.vectorInk.opacity(0.18), lineWidth: 1)
                .frame(width: radius * 1.84, height: radius * 1.84)
                .position(center)

            BikeSpokesShape(center: center, radius: radius)
                .stroke(Color.vectorDim.opacity(0.42), lineWidth: 0.8)
        }
    }
}
...
private struct BikeBatteryPack: View {
    ...
    var body: some View {
        ...
        RoundedRectangle(cornerRadius: thickness / 2, style: .continuous)
            .fill(
                LinearGradient(
                    colors: [Color.vectorVioletOnMap, Color.vectorViolet],
                    startPoint: .leading,
                    endPoint: .trailing
                )
            )
            .overlay {
                RoundedRectangle(cornerRadius: thickness / 2, style: .continuous)
                    .stroke(Color.vectorInk.opacity(0.46), lineWidth: 1)
            }
            .frame(width: length, height: thickness)
            .rotationEffect(angle)
            .shadow(color: Color.vectorViolet.opacity(0.42), radius: 6)
            .position(center)
    }
}
...
private struct BikeMotor: View {
    ...
    var body: some View {
        ZStack {
            Circle()
                .fill(Color.vectorMidnight.opacity(0.92))
            Circle()
                .stroke(Color.vectorVioletOnMap.opacity(0.9), lineWidth: 2)
            Image(systemName: "bolt.fill")
                .font(.system(size: 9, weight: .bold))
                .foregroundStyle(Color.routeTealOnMap)
        }
        .frame(width: 22, height: 22)
        .shadow(color: Color.vectorViolet.opacity(0.32), radius: 5)
        .position(center)
    }
}
